...
\tikzset{latent style/.style={row 1 column #1/.style={nodes={hidden}}}}
\tikzset{observation style/.style={row 2 column #1/.style={nodes={output}}}}
\matrix[matrix of math nodes,
  ...
  \textbf{Latent} &
    X_1 & X_2 &  \cdots & X_{T} &[10ex] X_t & X_{t+1}\\
  \textbf{Observed} &
   Y_1 & Y_2 &  \cdots & Y_{T} &[10ex] Y_t\\
};
\foreach \X in {2,3,4} {
  \draw[main edge] (m-1-\X) -- (m-1-\the\numexpr\X+1) node[midway,above]{};
  \ifnum\X=4
    \draw[main edge] (m-1-5) -- (m-2-5) node[pos=0.6,left]{};
  \else
    \draw[main edge] (m-1-\X) -- (m-2-\X) node[pos=0.6,left]{};
  \fi
 }
 \draw[main edge] (m-1-6) -- (m-1-7) node[pos=0.6, above=3mm]{$\prob_{\idxi\idxj}(t)$};
 \draw[main edge] (m-1-6) -- (m-2-6) node[pos=0.7,right]{$\emissionprob_{\idxi\idxk}(t)$};
